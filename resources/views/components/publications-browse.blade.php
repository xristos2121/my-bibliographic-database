            ...
            <div class="browse-results-item">
                ...
                <div class="publication-data-wrapper">
                        ...
                        <div class="publication-data-row">
                            ...
                            <span class="publication-data-value">
                                @foreach ($result->authors as $author)
                                    @php
                                        $authorName = urlencode($author->first_name . ' ' . $author->last_name);
                                        $searchUrl = url("/advanced-search/results?document_type=all&lookfor[0]=$authorName&type[0]=author");
                                    @endphp
                                    <a href="{{ $searchUrl }}">{{ $author->first_name }} {{ $author->last_name }}</a>{{ !$loop->last ? ', ' : '' }}
                                @endforeach
                            </span>
                        </div>
                    @endif
                    ...
                    @if ($result->hasCollections())
                        <div class="publication-data-row">
                            <span class="publication-data-label">Collections</span>
                            <span class="publication-data-value">
                                @foreach ($result->collections as $collection)
                                    <a href="{{ url('/browse/collections/' . $collection->slug) }}">{{ $collection->name }}</a>{{ !$loop->last ? ', ' : '' }}
                                @endforeach
                            </span>
                        </div>
                    @endif
