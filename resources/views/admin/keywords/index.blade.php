                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <div class="flex gap-4">
                            <form action="{{ route('keywords.index') }}" method="GET" class="flex items-center">
                                <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Search keywords..." class="px-4 py-2 border rounded-md" />
                                <button type="submit" class="ml-2 px-4 py-2 bg-blue-500 text-white rounded-md hover:bg-blue-700">Search</button>
                            </form>
                            <a href="{{ route('keywords.index') }}" class="inline-flex items-center px-4 py-2 bg-white text-black rounded-md  shadow">Clear all</a>
                            <a href="{{ route('keywords.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-500 text-white rounded-md hover:bg-blue-700">Add New Keyword</a>
                        </div>
                    </div>
                </div>
