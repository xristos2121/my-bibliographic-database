                ...
                <div class="px-6 py-4 bg-white border-b border-gray-200">
                    <div class="flex justify-between items-center">
                        <div class="flex-1 min-w-0">
                            <h2 class="text-2xl font-bold leading-7 text-gray-900 sm:text-3xl sm:truncate">Custom
                                Fields</h2>
                        </div>
                        <a href="{{ route('custom_fields.create') }}"
                ...
                <div class="px-6 py-4 bg-white border-b border-gray-200">
                    <form action="{{ route('custom_fields.index') }}" method="GET" class="flex items-center">
                        ...
                        <select name="type" class="w-fit py-2 mr-1 border rounded-md ml-2">
                            ...
                            <option
                                value="text" {{ request('type') === 'text' ? 'selected' : '' }}>{{ __('Text') }}</option>
                            <option
                                value="textarea" {{ request('type') === 'textarea' ? 'selected' : '' }}>{{ __('TextArea') }}</option>
                            <option
                                value="number" {{ request('type') === 'number' ? 'selected' : '' }}>{{ __('Number') }}</option>
                        </select>
                        <button type="submit"
                            ...
                            <tr>
                                ...
                                <td class="border px-4 py-2">
                                    <div class="flex items-center space-x-2">
                                        <a href="{{ route('custom_fields.edit', $field->id) }}"
                                        ...
                                        <form action="{{ route('custom_fields.destroy', $field->id) }}" method="POST"
                                              class="inline-flex">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
