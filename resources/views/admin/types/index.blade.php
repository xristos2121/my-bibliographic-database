                ...
                <div class="px-6 py-4 bg-white border-b border-gray-200">
                    <div class="flex justify-between items-center">
                        <div class="flex-1 min-w-0">
                            <h2 class="text-2xl font-bold leading-7 text-gray-900 sm:text-3xl sm:truncate">Publication Types</h2>
                        </div>
                        <div class="flex gap-4">
                            <form action="{{ route('publications_types.index') }}" method="GET" class="flex items-center">
                                <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Search publication types..." class="px-4 py-2 border rounded-md" />
                                <button type="submit" class="ml-2 px-4 py-2 bg-blue-500 text-white rounded-md hover:bg-blue-700">Search</button>
                            </form>
                            <a href="{{ route('publications_types.index') }}" class="inline-flex items-center px-4 py-2 bg-white text-black rounded-md  shadow">Clear all</a>
                            <a href="{{ route('publications_types.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-500 text-white rounded-md hover:bg-blue-700">Add New Type</a>
                        </div>
                    </div>
                </div>
